#Base effects + mortality fecundity effect beyond 0 probability
#Binary effect matrix for multiplying by effect size vector in regression model-----
tmt_spec_fx <- matrix(0, 
                      nrow = datapoints, 
                      ncol = lines + (drivers*sexes*lines) + length(construct_only) + timepoints + 1)
...
for(i in 1:datapoints){
  ...
  if(data_set[i,]$parental_mortality){
    tmt_spec_fx[i, timepoints + lines + length(construct_only) + (drivers*sexes*lines) + 1] <- 1
  }
}
...
beta_fx_key <- bind_rows(beta_fx_key,
                         data.frame(parental_mortality = T,
                                    beta_id = 1 + timepoints + length(construct_only) + lines + (drivers*sexes*lines)))
