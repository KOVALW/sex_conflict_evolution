...
fps <- paste0("fitting_code/", fps[which(grepl("soma_tmt", fps) & grepl(".R", fps))])
...
for (j in 1:length(fps)) {
  ...
  if(write_files) {
    ...
    parfit_summaries %>% 
      ...
      write_csv("./posterior_analysis/eta_summaries_soma.csv", append = file.exists("./posterior_analysis/eta_summaries_soma.csv") & j > 1)
    
    if (j > 1){
      bind_rows(read_csv("./posterior_analysis/beta_summaries_soma.csv"),
                parfit_summaries %>% 
        ...
        write_csv("./posterior_analysis/beta_summaries_soma.csv")
    } else {
      parfit_summaries %>% 
        filter(grepl("vals", par_name)) %>% 
        mutate(beta_id = as.numeric(substr(par_name, 11, nchar(par_name) - 1))) %>% 
    ...
    parfit_summaries %>% 
      ...
      mutate(model_index = j) %>% 
      write_csv("./posterior_analysis/base_par_summaries_soma.csv", 
                append = file.exists("./posterior_analysis/base_par_summaries_soma.csv") & j > 1)
...
looic_table %>% 
  ...
  write_csv("./posterior_analysis/looic_table_soma.csv")
